...
function M.lspSetup()
    ...
    G.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(ev)
            ...
            G.keymap.set('n', '<leader>wa', G.lsp.buf.add_workspace_folder, opts)
            G.keymap.set('n', '<leader>wr', G.lsp.buf.remove_workspace_folder, opts)
            G.keymap.set('n', '<leader>wl', function()
                print(G.inspect(G.lsp.buf.list_workspace_folders()))
            end, opts)
            G.keymap.set('n', '<leader>D', G.lsp.buf.type_definition, opts)
            G.keymap.set('n', '<F2>', G.lsp.buf.rename, opts)
            G.keymap.set({ 'n', 'v' }, '<leader>ca', G.lsp.buf.code_action, opts)
